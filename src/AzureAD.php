 ...
 * @license      MIT License
 */
namespace Opauth\AzureAD\Strategy;

use Opauth\Opauth\AbstractStrategy;

class AzureAD extends AbstractStrategy
{
    ...
    {
        $url = 'https://www.AzureAD.com/dialog/oauth';
        $strategyKeys = array(
            'scope',
            'state',
            'response_type',
    ...
    /**
     * Internal callback, after AzureAD's OAuth
     *
    ...
    public function callback()
    {
        ...
        $url = 'https://graph.AzureAD.com/oauth/access_token';
        ...
        $response->info['image'] = 'https://graph.AzureAD.com/' . $me['id'] . '/picture?type=square';
        return $response;
    }
    ...
    /**
     * Queries AzureAD Graph API for user info
     *
    ...
    {
        $me = $this->http->get('https://graph.AzureAD.com/me', array('access_token' => $access_token));
        if (empty($me)) {
            return false;
        }
